...
export function AppSidebar({ setTabs }) {
  const [sidebarItems, setSidebarItems] = useState([
    {
      ...
      subTopics: [
        { name: "Структура программы на С++", link: "/c-intro" },
  ...
  const filteredSidebarItems = sidebarItems.filter((item) => {
    ...
    const mappedInner = item.subTopics.filter((item) => {
      return item.name.toLowerCase().includes(searchText.toLowerCase());
    });

    if (mappedInner.length > 0) {
      return true;
    }

    return item.topic.name.toLowerCase().includes(searchText.toLowerCase());
  });
        ...
        <SidebarGroup className="text-lg">
          ...
          <SidebarGroupContent>
            <SidebarMenu className="flex flex-col">
              <SidebarMenuItem className="mb-2 px-2 box-content">
